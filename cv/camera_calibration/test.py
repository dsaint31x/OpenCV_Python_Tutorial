...
if __name__ == "__main__":
    ...
    # 투영 행렬 분해
    cameraMatrix, rotMatrix, transVect, rotMatrixX, rotMatrixY, rotMatrixZ, eulerAngles = cv.decomposeProjectionMatrix(proj_matrix)
    ...
    R, _ = cv.Rodrigues(r[0])
    camera_position = -np.dot(R.T, t[0])
    print(f'{camera_position.ravel() =}')
    
    # pan, tilt
    pan = np.arctan2(R[1, 0], R[0, 0]) * 180 / np.pi  # yaw
    tilt = np.arcsin(-R[2, 0]) * 180 / np.pi          # pitch

    print(f"Pan (yaw): {pan:.2f} degrees")
    print(f"Tilt (pitch): {tilt:.2f} degrees")
    
    
    ir = np.linalg.inv(R)
    arrayCampose = -ir@t[0]
    print("X="+str(arrayCampose[0]))
    print("Y="+str(arrayCampose[1]))
    print("Z="+str(arrayCampose[2]))
    ...
    print("pan =" + str(pan+2*math.pi)+"rad")
    print("tilt=" + str(tilt)+"rad")
    print("pan =" + str(pan*180/math.pi)+"degree")
    print("tilt=" + str(tilt*180/math.pi)+"degree")